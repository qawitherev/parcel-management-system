...
//             var res = await _client.PostAsync("api/v1/user/register/resident", content);
...
//             var res = await _client.PostAsync("api/v1/user/register/resident", content);
//             var deserialized = await res.Content.ReadFromJsonAsync<UserResponseDto>(IntegrationMisc.GetJsonSerializerOptions());
