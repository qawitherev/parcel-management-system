...
ExecStart=/usr/bin/dotnet ${PROD_DIR}/ParcelManagement.Api.dll --urls http://localhost:${PROD_PORT}
Restart=always
RestartSec=10
...
ExecStart=/usr/bin/dotnet ${STAGING_DIR}/ParcelManagement.Api.dll --urls http://localhost:${STAGING_PORT}
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal
